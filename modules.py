...
class LoRALinear(nn.Module):
    def __init__(self, linear_layer, r=8, alpha=32):
        super().__init__()
        self.lora_linear = linear_layer   # <-- 이름에 lora 추가
        self.r = r
        self.alpha = alpha
    ...
    def forward(self, x):
        return self.lora_linear(x) + self.lora_B(self.lora_A(x)) * (self.alpha/self.r)



class MlpWithLoRA(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, r=8, alpha=32, drop=0.1):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        self.fc1 = LoRALinear(nn.Linear(in_features, hidden_features), r, alpha)
        self.act = nn.ReLU()
        self.fc2 = LoRALinear(nn.Linear(hidden_features, out_features), r, alpha)
        self.drop = nn.Dropout(drop)
            ...
            if input_dim == dim:
                ...
                self.patch_projectors.append(nn.Sequential(
                                                 Rearrange('b c (h p1) (w p2) -> b (h w) (p1 p2 c)', 
                                                    p1 = p, p2 = p),
                                                 Mlp(input_dim, dim, dim))) # Mlp(input_dim, dim, dim) 에서 MlpsithLoRA 로 변경!
            cur_res /= p
...
class CTCHeadFFN(nn.Module):
    """
    Transformer encoder 출력 (d_model) → FFN → (vocab+1) 로그 확률
    """
    def __init__(self, d_model, vocab_size, d_ff=1024, dropout=0.2):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(d_model, d_ff),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(d_ff, vocab_size + 1),  # +1 for blank
        )
    def forward(self, x):
        # x: [B, T, d_model]
        return F.log_softmax(self.net(x), dim=-1)  # → [B, T, V+1]
