...
print(f'[DEBUG] Number of frames processed for face detection: {len(faces)}')

# ========== SCENE DETECTION ==========

scene = scene_detect(opt)
print(f'[DEBUG] Number of scenes detected: {len(scene)}')
...
for shot in scene:
  ...
  if shot[1].frame_num - shot[0].frame_num >= opt.min_track :
    ...
    print(f'[DEBUG] Processing shot with {len(scene_faces)} frames')
    new_tracks = track_shot(opt, scene_faces)
    print(f'[DEBUG] New tracks found in shot: {len(new_tracks)}')
    alltracks.extend(new_tracks)

# ========== FACE TRACK CROP ==========

for ii, track in enumerate(alltracks):
  print(f'[DEBUG] Cropping video for track #{ii}, frames: {len(track["frame"])}')
...
with open(savepath, 'wb') as fil:
  print(f'[DEBUG] Total vidtracks saved: {len(vidtracks)}')
